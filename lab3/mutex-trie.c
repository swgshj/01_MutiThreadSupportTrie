...
void init(int numthreads) {
  printf("NOW STARTING MULTITHREADING");
  root = NULL;
}
...
int _insert (const char *string, size_t strlen, int32_t ip4_address, 
  ...
  if (cmp == 0) {
	...
	return 1;
      ...
      {
	printf("Now in funny business");fflush(stdout);
       return 0;
   }
...
int insert (const char *string, size_t strlen, int32_t ip4_address)
{
  ...
  if (!string || !*string || !strlen)
   ...
   pthread_mutex_lock(&mutex);
   while(_search(root,string, strlen))
   {
 	printf("What you are inserting already exists");fflush(stdout);
 	pthread_cond_wait(&condition, &mutex);
   }    
  /*
     logic for insertion is below
   */
